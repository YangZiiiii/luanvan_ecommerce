                  ...
                  fetch(`http://localhost:8080/api/v1/cart/add?userId=${userId}&productId=${product.id}&quantity=1`, {
                     ...
                     .then(data => {
                        if (data && data.statusCode === 201) {
                           ...
                           // Cập nhật lại số lượng giỏ hàng trên icon sau khi thêm thành công
                           // Gọi lại API lấy giỏ hàng mới nhất
                           fetch(`http://localhost:8080/api/v1/cart?userId=${userId}`)
                              ...
                              .then(cartData => {
                                 if (
                                    ...
                                    updateCartNotify();
                                    window.dispatchEvent(new Event('cart-updated')); // Trigger event to update cart icon
                                 }
                              })
                              .catch(() => {
                                 updateCartNotify();
